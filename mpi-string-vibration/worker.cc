...
#pragma omp parallel for simd
    for (long i = start_segment; i < last_segment; i++) {
      ...
      d_t1[i] = L_x * (d_t[i + 1] + d_t[i - 1]) +
                2.0f * (1.0f - L_x) * (d_t[i]) -
                d_t1[i]; // The algorithm calls for d(i, t-1) here, but that is
                         // currently contained in d_t1
    }
    float *temp = d_t1;
    d_t1 = d_t;
    ...
    MPI_Status status;
    int next_node = rank + 1;
    int prev_node = rank - 1;
    if (rank == 0) {
      MPI_Sendrecv(&d_t[last_segment - 1], 1, MPI_FLOAT, next_node, 0,
                   &d_t[last_segment], 1, MPI_FLOAT, next_node, 0,
                   MPI_COMM_WORLD, &status);
    } else if (rank == world_size - 1) {

      MPI_Sendrecv(&d_t[start_segment], 1, MPI_FLOAT, prev_node, 0,
                   &d_t[start_segment - 1], 1, MPI_FLOAT, prev_node, 0,
                   MPI_COMM_WORLD, &status);
    } else {
      MPI_Sendrecv(&d_t[last_segment - 1], 1, MPI_FLOAT, next_node, 0,
                   &d_t[last_segment], 1, MPI_FLOAT, next_node, 0,
